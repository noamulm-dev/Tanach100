...
import { fetchFullBook, cleanHebrewText } from '../services/bibleService';

// Helper functions (moved from searchService)
...
self.onmessage = async (e: MessageEvent<SearchMessage>) => {
    if (e.data.type === 'START') {
        const { query, wholeWord, scope, currentBookId } = e.data.payload;

        try {
            const trimmedQuery = query.trim();
            if (!trimmedQuery) {
                self.postMessage({ type: 'RESULTS', results: [], regularSearchEnabled: false });
                return;
            }
            ...
            if (shouldDoElsSearch) {
                ...
                if (cleanWords.length > 0) {
                    ...
                    const totalIterations = cleanWords.length * skipsForEls.length;
                    ...
                    cleanWords.forEach((cleanWord, wordIdx) => {
                        ...
                        skipsForEls.forEach((skip) => {
                            ...
                            if (now - lastProgressUpdate > 100) {
                                const percent = Math.round((processedIterations / totalIterations) * 100);
            ...
            self.postMessage({
                type: 'RESULTS',
                results,
                regularSearchEnabled: shouldDoRegularSearch
            });
